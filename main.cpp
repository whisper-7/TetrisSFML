...
class UI {
private:
    static UI* instance;    // 静态实例指针
    Font& font;
    sf::Music backgroundMusic;
    sf::SoundBuffer lineClearBuffer;  // 消除行音效
    sf::Sound lineClearSound;
    sf::SoundBuffer rotateBuffer;     // 旋转音效
    sf::Sound rotateSound;
    sf::SoundBuffer moveBuffer;      // 新增：移动音效
    sf::Sound moveSound;
    sf::SoundBuffer dropBuffer;      // 新增：快速下落音效
    sf::Sound dropSound;
    sf::SoundBuffer gameStartBuffer;    // 新增：开始游戏音效
    sf::Sound gameStartSound;
    sf::SoundBuffer gameOverBuffer;     // 新增：游戏结束音效
...
public:
    ...
    // 非静态版本（供内部使用）
    void playLineClearSoundNonStatic() {
        lineClearSound.play();
    }

    void playRotateSoundNonStatic() {
        rotateSound.play();
    }
...
void resetGame() {
    ...
    loadHighScore();
    UI::resumeMusic();  // 或者用 UI::playMusic()
    UI::playGameStartSound();
